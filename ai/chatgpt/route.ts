// app/api/ai/chatgpt/route.ts
/**
 * CR Ahzamycode
 * ChatGPT (gpt-5-nano) via chatgptfree.ai – nonce dinamis
 * GET  : ?prompt=<teks>
 * POST : JSON {prompt:<teks>}
 */
...
async function getNonce(): Promise<string> {
  // 1. ambil halaman utama untuk dapat nonce
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: { reply, model: 'gpt-5-nano' }, timestamp: new Date().toISOString() });
  } catch (e: any) {
