/**
 * CR Ahzamycode
 * Handler untuk /api/ai/text2video
 * prompt → video 3-5 detik (aiLabs)
 */
...
/* ---------- config ---------- */
const BASE = 'https://text2video.aritek.app';
const ENDPOINTS = {
...
export async function GET(req: NextRequest) {
  ...
  if (!/^[a-zA-Z0-9\s.,!?'-]+$/.test(prompt))
    return NextResponse.json({ statuss: false, error: 'Prompt mengandung karakter tidak diizinkan', code: 400 }, { status: 400 });
