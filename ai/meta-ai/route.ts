...
async function metaAi(prompt: string) {
...
export async function GET(req: NextRequest) {
  ...
  try {
    const reply = await metaAi(prompt);
    return NextResponse.json({ success: true, data: { reply, model: 'meta-ai' }, timestamp: new Date().toISOString() });
  } catch (e: any) {
